...
for (let i = 0; i < 3; i++) {
    ...
    if (i == 0) snake[i].timeout = 100 - speedRed.value;
    if (i == 1) snake[i].timeout = 100 - speedGreen.value;
    if (i == 2) snake[i].timeout = 100 - speedBlue.value;
    snake[i].id = i;
    snake[i].light = 0
    // Head
    const materialHead = new THREE.MeshStandardMaterial({ flatShading: true });
    materialHead.emissive.setHSL(i / 3, 0.7, 0.5)
    materialHead.color.setHSL(i / 3, 0.7, 0.5)
...
speedRed.oninput = () => snake[0].timeout = 100 - speedRed.value;
speedGreen.oninput = () => snake[1].timeout = 100 - speedGreen.value;
speedBlue.oninput = () => snake[2].timeout = 100 - speedBlue.value;
...
function createBlock(block) {
    ...
    for (let i = 1; i < this.blocks.length; i++) {
        if (this.blocks[i].mesh) {
            let color = new THREE.Color().setHSL(this.id / 3, 0.7, (Math.sin(i) + 11) / 20 + this.light * 0.3)
...
setInterval(() => {
    scoreRed.innerHTML = snake[0].blocks.length
    scoreGreen.innerHTML = snake[1].blocks.length
    scoreBlue.innerHTML = snake[2].blocks.length
}, 50);